...
def read_text(batch_size=128): # mecab, bp를 다한것을 넣어야 한다.
...
def to_text(indice, vocab):
    ...
    for i in range(len(indice)):
        ...
        line = ' '.join(line) # 한 문장을 만들고
        lines += [line] # 문장들의 리스트에 넣는다.
...
if __name__ == '__main__':
    ...
    if config.gpu_id >= 0: # gpu id에 따라서 모델을 옮겨준다.
    ...
    with torch.no_grad():
        ...
        for lines in read_text(batch_size=config.batch_size):
            ...
            lengths         = [len(line) for line in lines] # 샘플별길이
            original_indice = [i for i in range(len(lines))] # 인덱스정보를 담고있다.

            sorted_tuples = sorted( # 정렬을 가장큰것을 맨위로 올린다.
            ...
            # |x| = (batch_size, length) -> 원학 벡터로 만들어 준것이다.
            ...
            if config.beam_size == 1:
                ...
                sorted_tuples = sorted(zip(output, original_indice), key=itemgetter(1)) # 앞서 길이기 긴것을 위로 정려했기때문에 원래의 index로 다시 정렬한다.
                output = [sorted_tuples[i][0] for i in range(len(sorted_tuples))] # 거기에서 우리가 원하는 출력만 받아온다.

                sys.stdout.write('\n'.join(output) + '\n') # 그것을 stdout으로 써주고 배치별로 나오게 될 것이디/
            else:
